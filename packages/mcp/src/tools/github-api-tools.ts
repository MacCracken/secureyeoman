...
export function registerGithubApiTools(
  ...
  server.registerTool(
    'github_setup_ssh',
    {
      ...
      inputSchema: {
        title: z
          .string()
          .describe(
            'Label shown in GitHub Settings > SSH Keys (e.g. "secureyeoman-container" or "prod-mcp-2024")'
          ),
      },
    },
    wrapToolHandler('github_setup_ssh', middleware, async (args) => {
      ...
      const secretName = `GITHUB_SSH_${args.title.toUpperCase().replace(/[^A-Z0-9]/g, '_')}`;
  ...
  server.registerTool(
    'github_rotate_ssh_key',
    {
      ...
      inputSchema: {
        ...
        old_key_id: z
          ...
          .int()
          .optional()
          .describe('GitHub key ID to revoke — defaults to stored ID from last github_setup_ssh'),
      },
    },
    wrapToolHandler('github_rotate_ssh_key', middleware, async (args) => {
      ...
      let oldKeyId: number | null = args.old_key_id ?? null;
      if (!oldKeyId) {
        try {
          const meta = JSON.parse(await readFile(metaPath, 'utf8')) as Record<string, unknown>;
          oldKeyId = meta.githubKeyId as number;
        } catch {
          /* meta missing after restart — user must supply old_key_id */
        }
      }
      ...
      // Revoke old key
      let revokeNote =
        'no old key ID available — old key left in GitHub (remove manually via github_list_ssh_keys + github_delete_ssh_key)';
      if (oldKeyId) {
        try {
      ...
      try {
        ...
        /* meta may have been overwritten — ok */
      }

      const newSecretName = `GITHUB_SSH_${args.title.toUpperCase().replace(/[^A-Z0-9]/g, '_')}`;
      ...
      if (tokenSecret) {
        ...
        if (oldSecretName && oldSecretName !== newSecretName) {
          try {
            ...
            /* old secret may not exist — ignore */
          }
        }
      }
  ...
  server.registerTool(
    'github_sync_fork',
    {
      ...
      inputSchema: {
        ...
        base: z
          .string()
          .describe('Branch in your fork to receive upstream changes (e.g. "main" or "master")'),
        head: z
          .string()
          .optional()
          .describe(
            'Upstream branch to merge in — use "upstream:main" notation or just the branch name (defaults to the upstream default branch)'
          ),
        commit_message: z
          .string()
          .optional()
          .describe('Custom commit message for the merge commit (optional)'),
      },
    },
    wrapToolHandler('github_sync_fork', middleware, async (args) => {
      const result = await client.post(
        `/api/v1/github/repos/${args.owner}/${args.repo}/sync-fork`,
        {
          base: args.base,
          head: args.head,
