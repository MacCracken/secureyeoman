...
export function getToolManifest(): ToolManifestEntry[] {
  return [
    ...
    { name: 'network_device_connect', description: 'Open SSH session to a network device; returns sessionId for subsequent commands' },
    { name: 'network_show_command', description: 'Execute IOS-XE/NX-OS/IOS-XR/EOS show commands on a connected device' },
    { name: 'network_config_push', description: 'Push configuration lines to a device via SSH config mode (dry-run supported)' },
    { name: 'network_health_check', description: 'Fleet-wide health check: run show version + show interfaces across a list of targets' },
    { name: 'network_ping_test', description: 'Execute ping from a connected device to a target IP; returns loss % and RTT' },
    { name: 'network_traceroute', description: 'Execute traceroute from a connected device; returns hop list with latency' },

    // Network tools — discovery & topology (46.2)
    { name: 'network_discovery_cdp', description: 'Run show cdp neighbors detail; return structured neighbor list' },
    { name: 'network_discovery_lldp', description: 'Run show lldp neighbors detail; return structured neighbor list' },
    { name: 'network_topology_build', description: 'Recursively discover network topology via CDP from seed devices; returns JSON graph and Mermaid diagram' },
    { name: 'network_arp_table', description: 'Return parsed ARP table (IP → MAC → interface) from a connected device' },
    { name: 'network_mac_table', description: 'Return parsed MAC address table (MAC → VLAN → interface) from a switch' },

    // Network tools — routing & switching (46.3)
    { name: 'network_routing_table', description: 'Parse show ip route; return structured route entries with protocol, prefix, next-hop, AD/metric' },
    { name: 'network_ospf_neighbors', description: 'Parse show ip ospf neighbor; return neighbor list with state, dead timer, interface' },
    { name: 'network_ospf_lsdb', description: 'Parse show ip ospf database; return LSA summary by type' },
    { name: 'network_bgp_peers', description: 'Parse show bgp summary; return peer list with ASN, state, prefix count' },
    { name: 'network_interface_status', description: 'Parse show interfaces; return per-interface admin/oper state, speed, duplex, errors' },
    { name: 'network_vlan_list', description: 'Parse show vlan brief; return VLAN ID, name, active ports' },

    // Network tools — security auditing (46.4)
    { name: 'network_acl_audit', description: 'Parse show ip access-lists; return ACL entries, match counts, implicit deny analysis' },
    { name: 'network_aaa_status', description: 'Return AAA server list and method config from a connected device' },
    { name: 'network_port_security', description: 'Parse show port-security; return per-interface violations, max MAC, sticky config' },
    { name: 'network_stp_status', description: 'Parse show spanning-tree; return root bridge, port roles/states, topology change count' },
    { name: 'network_software_version', description: 'Parse show version; return OS family, version string, uptime, platform, serial number' },

    // Network tools — NetBox source of truth (46.5)
    { name: 'netbox_devices_list', description: 'Query NetBox devices with optional site/role/tag/status filters' },
    { name: 'netbox_interfaces_list', description: 'Query NetBox interfaces for a device with IP assignments' },
    { name: 'netbox_ipam_ips', description: 'Query NetBox IP addresses by prefix, VRF, or device' },
    { name: 'netbox_cables', description: 'Query NetBox cables with endpoint A/B device and interface' },
    { name: 'netbox_reconcile', description: 'Compare live CDP topology against NetBox cables; return structured drift report' },

    // Network tools — NVD / CVE vulnerability assessment (46.6)
    { name: 'nvd_cve_search', description: 'Search NVD CVE database by keyword with optional CVSS severity filter' },
    { name: 'nvd_cve_by_software', description: 'Look up CVEs for a specific vendor/product/version using CPE match' },
    { name: 'nvd_cve_get', description: 'Fetch full CVE record by CVE ID including CVSS v3 vector, CWE, and references' },

    // Network tools — utilities (46.7)
    { name: 'subnet_calculator', description: 'Calculate IPv4 subnet details: network, broadcast, first/last host, mask, wildcard mask, host count' },
    { name: 'subnet_vlsm', description: 'VLSM planning — carve a parent prefix into subnets sized for given host requirements' },
    { name: 'wildcard_mask_calc', description: 'Convert a subnet mask or prefix length to an ACL wildcard mask' },

    // Network tools — PCAP analysis (46.8)
    { name: 'pcap_upload', description: 'Upload a pcap/pcapng file (base64-encoded) for analysis; returns pcapId' },
    { name: 'pcap_protocol_hierarchy', description: 'Run tshark protocol hierarchy statistics on an uploaded pcap' },
