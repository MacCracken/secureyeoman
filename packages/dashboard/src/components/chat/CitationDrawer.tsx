    ...
    <div className="fixed inset-y-0 right-0 w-96 bg-white dark:bg-gray-900 shadow-xl z-50 flex flex-col border-l border-gray-200 dark:border-gray-700">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        {source.documentTitle && (
          <div>
            <p className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">Document</p>
        ...
        <div>
          ...
          <span className={`inline-block px-2 py-0.5 rounded text-xs ${
            source.type === 'web_search' ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' :
            source.type === 'document_chunk' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300' :
            source.type === 'memory' ? 'bg-purple-100 text-purple-700 dark:bg-purple-900 dark:text-purple-300' :
        ...
        {source.url && (
          <div>
            ...
            <a href={source.url} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600 hover:underline break-all">
              {source.url}
            </a>
          </div>
        )}
        ...
        {source.trustScore != null && (
          <div>
            <p className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide">Trust Score</p>
            <p className="text-sm">{(source.trustScore * 100).toFixed(0)}%</p>
          </div>
        )}

        <div>
          <p className="text-xs text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1">Content</p>
