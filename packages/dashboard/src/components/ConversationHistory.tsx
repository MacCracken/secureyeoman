...
import { fetchConversationHistory, sealConversationTopic, fetchCompressedContext, type HistoryEntry } from '../api/client';
import { sanitizeText } from '../utils/sanitize';

const TIER_CONFIG = {
  message: { label: 'Messages', color: 'border-green-500/30 bg-green-500/5', icon: MessageSquare, textColor: 'text-green-400' },
  topic: { label: 'Topics', color: 'border-blue-500/30 bg-blue-500/5', icon: BookOpen, textColor: 'text-blue-400' },
  bulk: { label: 'Bulk', color: 'border-gray-500/30 bg-gray-500/5', icon: Archive, textColor: 'text-gray-400' },
};
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-2">
        {(['message', 'topic', 'bulk'] as const).map((tier) => {
          const tierEntries = tier === 'message' ? messageEntries : tier === 'topic' ? topicEntries : bulkEntries;
          const tokens = tierEntries.reduce((sum: number, e: HistoryEntry) => sum + e.tokenCount, 0);
            ...
            <div key={tier} className={`rounded-lg border p-2 ${config.color}`}>
              ...
              <div className="mt-1 h-1 bg-muted rounded-full overflow-hidden">
                <div
                  ...
                  style={{ width: `${totalTokens > 0 ? Math.min(100, (tokens / totalTokens) * 100) : 0}%` }}
                />
              </div>
            </div>
          );
      ...
      {showContext && contextData && (
        <div className="bg-muted/20 rounded-lg p-3 border text-xs space-y-2">
          <div className="font-medium">Compressed Context ({contextData.totalTokens ?? 0} tokens)</div>
          <div className="text-muted-foreground">
            Budget: msgs={contextData.tokenBudget?.messages ?? 0}, topics={contextData.tokenBudget?.topics ?? 0}, bulk={contextData.tokenBudget?.bulk ?? 0}
          </div>
        </div>
      )}
          ...
          return (
            <div key={entry.id} className={`rounded border ${config.color}`}>
              ...
              >
                ...
                <Icon className={`w-3 h-3 ${config.textColor}`} />
                <span className="text-xs truncate flex-1">{sanitizeText(entry.content.substring(0, 80))}</span>
                <span className="text-[10px] text-muted-foreground">{entry.tokenCount}t</span>
                {entry.sealedAt && (
                  <span className="text-[10px] text-muted-foreground bg-muted px-1 rounded">sealed</span>
                )}
              </button>
              {isExpanded && (
