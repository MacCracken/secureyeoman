...
import { ShieldCheck, BarChart3, Sparkles, Database, FileText, Search, AlertTriangle } from 'lucide-react';
...
async function fetchCohortAnalysis(evalRunId: string): Promise<{ slices: CohortSlice[] }> {
  const res = await fetch(`/api/v1/responsible-ai/cohort-analysis?evalRunId=${encodeURIComponent(evalRunId)}`, {
    headers: API_HEADERS(),
  });
...
async function fetchFairness(evalRunId: string): Promise<FairnessMetrics> {
  const res = await fetch(`/api/v1/responsible-ai/fairness?evalRunId=${encodeURIComponent(evalRunId)}`, {
    headers: API_HEADERS(),
  });
...
async function fetchProvenance(datasetId: string): Promise<ProvenanceSummary> {
  const res = await fetch(`/api/v1/responsible-ai/provenance/summary/${encodeURIComponent(datasetId)}`, {
    headers: API_HEADERS(),
  });
...
async function redactProvenance(datasetId: string, entryId: string): Promise<void> {
  const res = await fetch(`/api/v1/responsible-ai/provenance/${encodeURIComponent(datasetId)}/redact/${encodeURIComponent(entryId)}`, {
    method: 'POST',
    headers: API_HEADERS(),
  });
  ...
  return (
    <Section title="Cohort Error Analysis" icon={<BarChart3 className="w-5 h-5 text-blue-400" />}>
      <div className="flex gap-2 mb-4">
        ...
        />
        <button
          onClick={() => setSubmittedId(evalRunId)}
      ...
      {error && <p className="text-red-400 text-sm">Error: {(error as Error).message}</p>}
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody>
              {data.slices.map((slice, i) => (
                <tr key={`${slice.dimension}-${slice.value}-${i}`} className={`border-b border-zinc-800 ${errorRateBg(slice.errorRate)}`}>
                  ...
                  <td className={`py-2 px-3 text-right font-mono font-semibold ${errorRateColor(slice.errorRate)}`}>
                    {(slice.errorRate * 100).toFixed(1)}%
                  </td>
                  <td className="py-2 px-3 text-right text-zinc-300">{slice.avgScore.toFixed(3)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-white font-mono text-sm">{data.disparateImpactRatio.toFixed(3)}</span>
            </div>
            ...
            <p className="text-zinc-500 text-xs mt-1">Acceptable range: 0.8 - 1.25 (four-fifths rule)</p>
          </div>
              ...
              <tbody>
                {data.groups.map((g) => (
                  <tr key={g.name} className="border-b border-zinc-800">
                    ...
                    <td className="py-2 px-3 text-right text-zinc-300 font-mono">{(g.positiveRate * 100).toFixed(1)}%</td>
    ...
    <Section title="SHAP Explainability" icon={<Sparkles className="w-5 h-5 text-amber-400" />}>
      <div className="flex gap-2 mb-4">
        <input
          ...
          value={shapId}
          onChange={(e) => setShapId(e.target.value)}
          className="flex-1 bg-zinc-800 border border-zinc-700 rounded px-3 py-2 text-sm text-white placeholder-zinc-500"
        />
        <button
          onClick={() => setSubmittedId(shapId)}
      ...
      {data && (
        <div>
          ...
          <div className="flex items-center gap-4 mt-3 text-xs text-zinc-500">
            ...
            <div className="flex items-center gap-1">
              <div className="w-3 h-3 rounded" style={{ backgroundColor: 'rgba(59, 130, 246, 0.25)' }} />
              Low negative
            </div>
            <div className="flex items-center gap-1">
  ...
  return (
    <Section title="Data Provenance" icon={<Database className="w-5 h-5 text-teal-400" />}>
      <div className="flex gap-2 mb-4">
        ...
        />
        <button
          onClick={() => setSubmittedId(datasetId)}
              ...
              <tbody>
                {filteredEntries.map((entry) => (
                  <tr key={entry.id} className="border-b border-zinc-800">
                    ...
                    <td className="py-2 px-3 text-zinc-300 font-mono text-xs">{entry.conversationId}</td>
        ...
        <div className="space-y-4">
          ...
          {Object.keys(data.metrics).length > 0 && (
            <div className="bg-zinc-800 rounded-lg p-4">
              <p className="text-zinc-400 text-xs font-semibold uppercase mb-2">Performance Metrics</p>
    ...
    <div className="p-6 max-w-6xl mx-auto">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          ...
          <p className="text-zinc-400 text-sm">Bias detection, fairness analysis, explainability, provenance, and model documentation</p>
        </div>
      </div>
