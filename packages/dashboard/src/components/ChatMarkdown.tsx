  ...
  const components = useMemo(
    () => ({
      ...
      h1: ({ children }: { children?: React.ReactNode }) => (
        <h1
          className={`font-bold ${isSm ? 'text-base' : 'text-sm'} mt-3 mb-1 border-b border-muted-foreground/20 pb-1`}
        >
          {children}
        </h1>
      ),
      ...
      ul: ({ children, className }: { children?: React.ReactNode; className?: string }) => {
        ...
        return (
          <ul
            className={
              isTaskList ? 'space-y-1 pl-1 list-none' : 'list-disc list-inside space-y-0.5 pl-2'
            }
          >
            {children}
          </ul>
      ...
      li: ({ children, className }: { children?: React.ReactNode; className?: string }) => {
        ...
        return (
          <li
            className={
              isTask ? 'flex items-start gap-2 leading-relaxed list-none' : 'leading-relaxed'
            }
          >
            {children}
          </li>
      ...
      blockquote: ({ node, children }: { node?: unknown; children?: React.ReactNode }) => {
        ...
        if (alertType) {
          ...
          return (
            <div
              className={`border-l-4 ${config.borderClass} ${config.bgClass} pl-3 pr-2 py-2 rounded-r my-2 space-y-1`}
            >
              <div
                className={`font-semibold ${config.textClass} ${textSmall} flex items-center gap-1.5`}
              >
