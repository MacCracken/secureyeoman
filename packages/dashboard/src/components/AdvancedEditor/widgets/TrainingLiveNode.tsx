...
import { fetchTrainingStream, fetchQualityScores, triggerQualityScoring } from '../../../api/client';

interface StreamPoint { ts: number; value: number; }
    ...
    <div className="p-3 space-y-3 text-sm h-full overflow-auto">
      <div className="grid grid-cols-2 gap-2">
        <div className="rounded border p-2">
          <div className="text-[10px] text-muted-foreground flex items-center gap-1"><Zap className="w-3 h-3" /> Throughput</div>
        ...
        <div className="rounded border p-2">
          <div className="text-[10px] text-muted-foreground flex items-center gap-1"><Activity className="w-3 h-3" /> Agreement</div>
        ...
        <div>
          ...
          <ResponsiveContainer width="100%" height={100}>
            <LineChart data={lossSeries}>
              ...
              <Line type="monotone" dataKey="value" dot={false} strokeWidth={1.5} stroke="var(--color-primary, #6366f1)" />
            </LineChart>
          </ResponsiveContainer>
        </div>
      )}
