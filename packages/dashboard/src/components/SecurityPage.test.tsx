...
const mockFetchTlsStatus = vi.mocked(api.fetchTlsStatus);
const mockFetchAutonomyOverview = vi.mocked(api.fetchAutonomyOverview);
...
beforeEach(() => {
  ...
  mockFetchHeartbeatStatus.mockResolvedValue({ tasks: [], enabledTasks: 0, totalTasks: 0 } as any);
  vi.mocked(api.fetchPersonalities).mockResolvedValue({ personalities: [] });
  mockFetchTasks.mockResolvedValue({ tasks: [], total: 0 });
  mockFetchTlsStatus.mockResolvedValue({ enabled: false, autoGenerated: false, certPath: null, keyPath: null, caPath: null, expiresAt: null, daysUntilExpiry: null, expired: false, expiryWarning: false } as any);
  mockFetchAutonomyOverview.mockResolvedValue({ byLevel: {} as any, totals: {} as any });
...
describe('SecurityPage — Automations tab', () => {
  it('renders the Automations tab button', () => {
    renderWithRoute('/security');
    expect(screen.getByRole('button', { name: /automations/i })).toBeInTheDocument();
  });

  it('shows Automations tab content when ?tab=automations', async () => {
    renderWithRoute('/security?tab=automations');
    expect(await screen.findByRole('tablist', { name: 'Automations views' })).toBeInTheDocument();
  });
  ...
  it('shows empty task list message when switching to Tasks subview', async () => {
    renderWithRoute('/security?tab=automations');
    await screen.findByRole('tablist', { name: 'Automations views' });
  ...
  it('shows tasks after switching to Tasks subview', async () => {
  ...
  it('shows Heartbeats content as the default subview', async () => {
    renderWithRoute('/security?tab=automations');
    await screen.findByRole('tablist', { name: 'Automations views' });
