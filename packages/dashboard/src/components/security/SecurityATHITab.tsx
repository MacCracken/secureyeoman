...
function ScenarioModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" data-testid="athi-scenario-modal">
      ...
      <div className="relative bg-background border border-border rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto mx-4">
        <div className="sticky top-0 bg-background border-b border-border px-6 py-4 flex items-center justify-between z-10">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold">{initial ? 'Edit Scenario' : 'Create Threat Scenario'}</h2>
          </div>
        ...
        >
            ...
            <div className="flex flex-wrap gap-1.5">
              {TECHNIQUES.map((t) => (
                <button
                  key={t}
                  type="button"
                  onClick={() => setForm({ ...form, techniques: toggleItem(form.techniques, t) })}
            ...
            <div className="flex flex-wrap gap-1.5">
              {HARMS.map((h) => (
                <button
                  key={h}
                  type="button"
                  onClick={() => setForm({ ...form, harms: toggleItem(form.harms, h) })}
                  className={`px-2 py-1 text-xs rounded border transition-colors ${
            ...
            <div className="flex flex-wrap gap-1.5">
              {IMPACTS.map((i) => (
                <button
                  key={i}
                  type="button"
                  onClick={() => setForm({ ...form, impacts: toggleItem(form.impacts, i) })}
                  className={`px-2 py-1 text-xs rounded border transition-colors ${
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <div className={`flex items-center justify-center h-[38px] rounded text-sm font-bold ${riskColor(form.likelihood * form.severity)}`}>
          ...
          <div className="bg-background border border-border rounded-lg p-4">
            <div className="flex flex-wrap gap-1">
              {Object.entries(summary.byStatus ?? {}).map(([status, count]) => (
                ...
                >
                  {formatLabel(status)}: {count as number}
                </span>
              ))}
            </div>
                ...
                {scenarios.map((s: any) => (
                  <tr key={s.id} className="border-b border-border/50 hover:bg-muted/30">
                    ...
                    <td className="py-2 px-2 text-center">
                      <span className={`px-2 py-0.5 rounded text-xs font-bold ${riskColor(s.riskScore)}`}>
                        {s.riskScore}
                      </span>
                    </td>
                    <td className="py-2 px-2 text-center">
