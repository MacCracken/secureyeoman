...
export function serializeEncrypted(data) {
    ...
    const buffer = Buffer.alloc(MAGIC_BYTES.length +
        1 + // version
        1 + data.salt.length +
        1 + data.iv.length +
 ...
 */
export function deserializeEncrypted(buffer) {
    let offset = 0;
    // Check magic bytes
 ...
 */
export function createSecretStore(config) {
 ...
 */
export function decryptValue(encryptedBase64, key) {
