...
export class SpiritStorage extends PgBaseStorage {
  ...
  async getPassion(id: string): Promise<Passion | null> {
    const row = await this.queryOne<PassionRow>('SELECT * FROM spirit.passions WHERE id = $1', [
      id,
    ]);
  ...
  async deletePassion(id: string): Promise<boolean> {
    const count = await this.execute('DELETE FROM spirit.passions WHERE id = $1', [id]);
    return count > 0;
  }
  ...
  async deleteInspiration(id: string): Promise<boolean> {
    const count = await this.execute('DELETE FROM spirit.inspirations WHERE id = $1', [id]);
    return count > 0;
  }
  ...
  async getPain(id: string): Promise<Pain | null> {
    const row = await this.queryOne<PainRow>('SELECT * FROM spirit.pains WHERE id = $1', [id]);
  ...
  async deletePain(id: string): Promise<boolean> {
    const count = await this.execute('DELETE FROM spirit.pains WHERE id = $1', [id]);
    return count > 0;
  }
