...
export function registerExperimentRoutes(app: FastifyInstance, opts: ExperimentRoutesOptions): void {
  ...
  app.get('/api/v1/experiments', async () => { const experiments = await experimentManager.list(); return { experiments, total: experiments.length }; });
  ...
  app.post('/api/v1/experiments', async (request: FastifyRequest<{ Body: { name: string; description?: string; variants: unknown[] } }>, reply: FastifyReply) => {
    try { const exp = await experimentManager.create(request.body as any); return reply.code(201).send({ experiment: exp }); }
  ...
  app.delete('/api/v1/experiments/:id', async (request: FastifyRequest<{ Params: { id: string } }>, reply: FastifyReply) => {
    if (!await experimentManager.delete(request.params.id)) return reply.code(404).send({ error: 'Experiment not found' });
