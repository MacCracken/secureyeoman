...
describe('GenericWebhookIntegration', () => {
  ...
  it('should have platform "webhook"', () => {
    expect(adapter.platform).toBe('webhook');
  });

  it('should have rate limit of 30 per second', () => {
  ...
  describe('sendMessage()', () => {
    it('should POST to the webhook URL and return a wh_ prefixed ID', async () => {
    ...
    it('should throw when no webhook URL is configured', async () => {
    ...
    it('should omit signature header when no secret is configured', async () => {
      const mockFetch = vi.fn().mockResolvedValue({
        ok: true,
        text: () => Promise.resolve('ok'),
      });
  ...
  describe('getWebhookPath()', () => {
    it('should return webhook path containing the integration ID', async () => {
      await adapter.init(makeConfig(), makeDeps());
  ...
  describe('verifyWebhook()', () => {
    ...
    it('should reject an invalid signature', async () => {
      await adapter.init(makeConfig(), makeDeps());
      expect(adapter.verifyWebhook('payload', 'sha256=invalidsig')).toBe(false);
    });

    it('should accept any payload when no secret is configured', async () => {
      await adapter.init(
        makeConfig({ config: { webhookUrl: 'https://example.com/hook' } }),
        makeDeps()
      );
  ...
  describe('handleInbound()', () => {
    it('should normalize a full payload and call onMessage', async () => {
