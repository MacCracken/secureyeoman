...
describe('debug2', () => {
  ...
  beforeEach(async () => {
    ...
    bm = new BrainManager(
      ...
      { auditChain: null as any, logger: noopLogger() }
    );
  ...
  it('diagnoses large text corpus', async () => {
    ...
    console.log(
      ...
      chunks.length,
      'totalTokens:',
      chunks.reduce((s, c) => s + c.estimatedTokens, 0)
    );
    const corpus = await dm.getNotebookCorpus(null, 100);
    console.log(
      'corpus:',
      JSON.stringify({
        totalTokens: corpus.totalTokens,
        fitsInBudget: corpus.fitsInBudget,
        docsCount: corpus.documents.length,
      })
    );
  });
});
