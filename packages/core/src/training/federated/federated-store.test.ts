...
import type { FederatedSession, FederatedParticipant, FederatedRound, ModelUpdate } from '@secureyeoman/shared';
...
describe('FederatedStore', () => {
  ...
  it('gets a session by id', async () => {
    mockQuery.mockResolvedValueOnce({
      rows: [{ id: 'fl-1', name: 'Test', description: '', model_id: 'model-1',
        aggregation_strategy: 'fedavg', privacy: {}, min_participants: 2,
        max_rounds: 100, current_round: 0, convergence_threshold: 0.001,
        status: 'active', participant_ids: ['fp-1'], created_at: 1000,
        updated_at: 1000, tenant_id: 'default' }],
    });
  ...
  it('lists sessions with pagination', async () => {
    mockQuery
      ...
      .mockResolvedValueOnce({
        rows: [
          { id: 'fl-1', name: 'S1', status: 'active', created_at: 1000, updated_at: 1000, participant_ids: [], privacy: {}, tenant_id: 'default' },
          { id: 'fl-2', name: 'S2', status: 'active', created_at: 2000, updated_at: 2000, participant_ids: [], privacy: {}, tenant_id: 'default' },
        ],
      });
  ...
  it('lists participants with status filter', async () => {
    mockQuery.mockResolvedValueOnce({
      rows: [
        { id: 'fp-1', peer_id: 'p1', name: 'P1', status: 'active', dataset_size: 100, last_heartbeat: 1000, rounds_participated: 0, contribution_weight: 1, registered_at: 1000, tenant_id: 'default' },
      ],
    });
  ...
  it('saves a round', async () => {
    ...
    await store.saveRound({
      ...
      aggregationStrategy: 'fedavg', globalModelVersion: '', participantIds: ['fp-1'],
      updatesReceived: 0, updatesRequired: 1, privacy: {} as any,
  ...
  it('lists rounds for a session', async () => {
    mockQuery.mockResolvedValueOnce({
      rows: [
        { id: 'fr-1', round_number: 1, status: 'completed', aggregation_strategy: 'fedavg',
          global_model_version: '', participant_ids: [], updates_received: 2,
          updates_required: 2, privacy: {}, started_at: 1000, completed_at: 2000,
          created_at: 1000, tenant_id: 'default' },
      ],
    });
  ...
  it('saves and retrieves model updates', async () => {
    ...
    await store.saveModelUpdate({
      id: 'mu-1', participantId: 'fp-1', roundId: 'fr-1',
      gradientChecksum: 'abc', datasetSizeSeen: 100,
      trainingLoss: 0.5, metricsJson: { acc: 0.9 },
      submittedAt: 1000, privacyNoiseApplied: false,
    });
    const sql = mockQuery.mock.calls[0][0] as string;
  ...
  it('gets updates for a round', async () => {
    mockQuery.mockResolvedValueOnce({
      rows: [
        { participant_id: 'fp-1', round_id: 'fr-1', gradient_checksum: 'abc',
          dataset_size_seen: 100, training_loss: 0.5, validation_loss: null,
          metrics_json: { acc: 0.9 }, submitted_at: 1000, privacy_noise_applied: false },
      ],
    });
