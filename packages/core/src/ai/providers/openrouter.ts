 ...
 * OpenRouter Provider
 *
 * Uses the `openai` package with a custom baseURL targeting OpenRouter's
 * OpenAI-compatible API at https://openrouter.ai/api/v1.
 * Requires an OPENROUTER_API_KEY.
 *
 * Dynamic model discovery via the OpenRouter models endpoint.
 */
...
export class OpenRouterProvider extends BaseProvider {
   ...
   */
  static async fetchAvailableModels(apiKey?: string): Promise<OpenRouterModelInfo[]> {
  ...
  /**
   * Return known OpenRouter models when API is not reachable.
   */
  static getKnownModels(): OpenRouterModelInfo[] {
    return [
  ...
  private mapError(error: unknown): Error {
    if (error instanceof OpenAI.APIError) {
      if (error.status === 429) {
        return new RateLimitError('openrouter', undefined, error);
      }
      if (error.status === 400 && error.message.includes('token')) {
        return new TokenLimitError('openrouter', error);
      }
      if (error.status === 502 || error.status === 503) {
        return new ProviderUnavailableError('openrouter', error.status, error);
      }
      return new InvalidResponseError('openrouter', error.message, error);
    }
