 ...
 * LM Studio Provider
 *
 * Uses the `openai` package with a custom baseURL targeting LM Studio's
...
export class LMStudioProvider extends BaseProvider {
  readonly name: AIProviderName = 'lmstudio';
  ...
  constructor(config: ProviderConfig, logger?: SecureLogger) {
    ...
    this.client = new OpenAI({
      ...
      baseURL: this.modelConfig.baseUrl ?? LMSTUDIO_BASE_URL,
    });
  }
  ...
  private mapError(error: unknown): Error {
    if (error instanceof OpenAI.APIError) {
      if (error.status === 429) {
        return new RateLimitError('lmstudio', undefined, error);
      }
      if (error.status === 400 && error.message.includes('token')) {
        return new TokenLimitError('lmstudio', error);
      }
      if (error.status === 502 || error.status === 503) {
        return new ProviderUnavailableError('lmstudio', error.status, error);
      }
      return new InvalidResponseError('lmstudio', error.message, error);
    }
