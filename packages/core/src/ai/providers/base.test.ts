...
class TestProvider extends BaseProvider {
  ...
  testResolveModel(request: AIRequest) { return this.resolveModel(request); }
  testResolveMaxTokens(request: AIRequest) { return this.resolveMaxTokens(request); }
  testResolveTemperature(request: AIRequest) { return this.resolveTemperature(request); }
}
...
describe('BaseProvider', () => {
  describe('constructor', () => {
    ...
    it('accepts optional logger', () => {
      const logger = { info: vi.fn(), debug: vi.fn(), warn: vi.fn(), error: vi.fn(), child: vi.fn().mockReturnThis() };
  ...
  describe('chat', () => {
    ...
    it('retries on failure', async () => {
      ...
      const provider = new FailThenSucceedProvider(makeConfig({ retryConfig: { maxRetries: 3, baseDelayMs: 0, maxDelayMs: 0 } }));
      const response = await provider.chat({ messages: [{ role: 'user', content: 'hello' }], stream: false });
      expect(response.content).toBe('response text');
  ...
  describe('resolveTemperature', () => {
    it('returns request temperature when provided', () => {
      const provider = new TestProvider(makeConfig());
      const temp = provider.testResolveTemperature({ messages: [], stream: false, temperature: 0.1 });
