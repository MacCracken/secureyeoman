...
describe('GroqProvider', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    provider = new GroqProvider(makeConfig());
  ...
  describe('constructor', () => {
    it('should throw when no API key is provided', () => {
      ...
      expect(() => new GroqProvider({ model: config.model })).toThrow(
        'Groq provider requires GROQ_API_KEY'
      );
    });

    it('should construct successfully with an API key', () => {
      expect(() => new GroqProvider(makeConfig())).not.toThrow();
    });
  });
  ...
  describe('chat', () => {
    it('should map request and response', async () => {
      ...
      expect(response.id).toBe('chatcmpl-groq-123');
      expect(response.content).toBe('Hi from Groq!');
  ...
  describe('fetchAvailableModels', () => {
    ...
    it('should return models from groq endpoint', async () => {
      ...
      const models = await GroqProvider.fetchAvailableModels('test-key');
    ...
    it('should return empty array on fetch failure', async () => {
      ...
      const models = await GroqProvider.fetchAvailableModels('bad-key');
    ...
    it('should return empty array when no key provided', async () => {
      const models = await GroqProvider.fetchAvailableModels();
      expect(models).toEqual([]);
    });
  });

  describe('getKnownModels', () => {
    it('should return known Groq models', () => {
      const models = GroqProvider.getKnownModels();
