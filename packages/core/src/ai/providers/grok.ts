 ...
 * x.ai Grok Provider
 *
 * Uses the `openai` package with a custom baseURL targeting x.ai's
 * OpenAI-compatible API at https://api.x.ai/v1.
 * Requires an XAI_API_KEY.
 */
...
export class GrokProvider extends BaseProvider {
  readonly name: AIProviderName = 'grok';
  ...
  constructor(config: ProviderConfig, logger?: SecureLogger) {
    ...
    if (!apiKey) {
      throw new Error('Grok provider requires XAI_API_KEY');
    }
    this.client = new OpenAI({
      apiKey,
      timeout: this.modelConfig.requestTimeoutMs,
      baseURL: this.modelConfig.baseUrl ?? process.env.XAI_BASE_URL ?? GROK_BASE_URL,
    });
  }

  /**
   * Fetch available models from x.ai's OpenAI-compatible models endpoint.
   */
  static async fetchAvailableModels(apiKey?: string): Promise<GrokModelInfo[]> {
    const key = apiKey ?? process.env.XAI_API_KEY;
    if (!key) return [];
    try {
      const baseUrl = process.env.XAI_BASE_URL ?? GROK_BASE_URL;
  ...
  /**
   * Return known Grok models when API is not reachable.
   */
  static getKnownModels(): GrokModelInfo[] {
    return [
      { id: 'grok-3', ownedBy: 'xai' },
      { id: 'grok-3-mini', ownedBy: 'xai' },
      { id: 'grok-2-1212', ownedBy: 'xai' },
      { id: 'grok-2-vision-1212', ownedBy: 'xai' },
    ];
  }
  ...
  private mapError(error: unknown): Error {
    if (error instanceof OpenAI.APIError) {
      if (error.status === 429) {
        return new RateLimitError('grok', undefined, error);
      }
      if (error.status === 400 && error.message.includes('token')) {
        return new TokenLimitError('grok', error);
      }
      if (error.status === 502 || error.status === 503) {
        return new ProviderUnavailableError('grok', error.status, error);
      }
      return new InvalidResponseError('grok', error.message, error);
    }
