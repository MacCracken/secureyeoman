 ...
 * DeepSeek Provider
 *
 * Uses the `openai` package with a custom baseURL targeting DeepSeek's
...
export class DeepSeekProvider extends BaseProvider {
  ...
  constructor(config: ProviderConfig, logger?: SecureLogger) {
    ...
    if (!apiKey) {
      throw new Error('DeepSeek provider requires DEEPSEEK_API_KEY');
    }
    this.client = new OpenAI({
      apiKey,
      timeout: this.modelConfig.requestTimeoutMs,
      baseURL: this.modelConfig.baseUrl ?? process.env.DEEPSEEK_BASE_URL ?? DEEPSEEK_BASE_URL,
    });
  }

  /**
   * Fetch available models from DeepSeek's OpenAI-compatible models endpoint.
   */
  static async fetchAvailableModels(apiKey?: string): Promise<DeepSeekModelInfo[]> {
  ...
  /**
   * Return known DeepSeek models when API is not reachable.
   */
  static getKnownModels(): DeepSeekModelInfo[] {
    return [
      { id: 'deepseek-chat', ownedBy: 'deepseek' },
      { id: 'deepseek-coder', ownedBy: 'deepseek' },
  ...
  private mapError(error: unknown): Error {
    if (error instanceof OpenAI.APIError) {
      if (error.status === 429) {
        return new RateLimitError('deepseek', undefined, error);
      }
      if (error.status === 400 && error.message.includes('token')) {
        return new TokenLimitError('deepseek', error);
      }
      if (error.status === 502 || error.status === 503) {
        return new ProviderUnavailableError('deepseek', error.status, error);
      }
      return new InvalidResponseError('deepseek', error.message, error);
    }
