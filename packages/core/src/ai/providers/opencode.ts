 ...
 *
 * Uses the `openai` package with a custom baseURL targeting OpenCode's
...
export class OpenCodeProvider extends BaseProvider {
  ...
  private mapError(error: unknown): Error {
    if (error instanceof OpenAI.APIError) {
      if (error.status === 429) {
        return new RateLimitError('opencode', undefined, error);
      }
      if (error.status === 401) {
        return new AuthenticationError('opencode', error);
      }
      if (error.status === 400 && error.message.includes('token')) {
        return new TokenLimitError('opencode', error);
      }
      if (error.status === 502 || error.status === 503) {
        return new ProviderUnavailableError('opencode', error.status, error);
      }
      return new InvalidResponseError('opencode', error.message, error);
    }
