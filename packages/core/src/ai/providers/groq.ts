 ...
 * Groq Provider
 *
 * Uses the `openai` package with a custom baseURL targeting Groq's
...
export class GroqProvider extends BaseProvider {
  ...
  constructor(config: ProviderConfig, logger?: SecureLogger) {
    ...
    this.client = new OpenAI({
      ...
      baseURL: this.modelConfig.baseUrl ?? process.env.GROQ_BASE_URL ?? GROQ_BASE_URL,
    });
  }

  /**
   * Fetch available models from Groq's OpenAI-compatible models endpoint.
   */
  static async fetchAvailableModels(apiKey?: string): Promise<GroqModelInfo[]> {
  ...
  /**
   * Return known Groq models when API is not reachable.
   */
  static getKnownModels(): GroqModelInfo[] {
    return [
      { id: 'llama-3.3-70b-versatile', ownedBy: 'groq' },
      { id: 'llama-3.1-8b-instant', ownedBy: 'groq' },
      { id: 'mixtral-8x7b-32768', ownedBy: 'groq' },
      { id: 'gemma2-9b-it', ownedBy: 'groq' },
    ];
  }
  ...
  private mapError(error: unknown): Error {
    if (error instanceof OpenAI.APIError) {
      if (error.status === 429) {
        return new RateLimitError('groq', undefined, error);
      }
      if (error.status === 400 && error.message.includes('token')) {
        return new TokenLimitError('groq', error);
      }
      if (error.status === 502 || error.status === 503) {
        return new ProviderUnavailableError('groq', error.status, error);
      }
      return new InvalidResponseError('groq', error.message, error);
    }
