 ...
 * Mistral AI Provider
 *
 * Uses the `openai` package with a custom baseURL targeting Mistral's
 * OpenAI-compatible API at https://api.mistral.ai/v1.
 * Requires a MISTRAL_API_KEY.
 */
...
export class MistralProvider extends BaseProvider {
  ...
  constructor(config: ProviderConfig, logger?: SecureLogger) {
    ...
    if (!apiKey) {
      throw new Error('Mistral provider requires MISTRAL_API_KEY');
    }
    this.client = new OpenAI({
      apiKey,
      timeout: this.modelConfig.requestTimeoutMs,
      baseURL: this.modelConfig.baseUrl ?? process.env.MISTRAL_BASE_URL ?? MISTRAL_BASE_URL,
    });
  }

  /**
   * Fetch available models from Mistral's OpenAI-compatible models endpoint.
   */
  static async fetchAvailableModels(apiKey?: string): Promise<MistralModelInfo[]> {
  ...
  /**
   * Return known Mistral models when API is not reachable.
   */
  static getKnownModels(): MistralModelInfo[] {
    return [
      { id: 'mistral-large-latest', ownedBy: 'mistralai' },
      { id: 'mistral-medium-latest', ownedBy: 'mistralai' },
      { id: 'mistral-small-latest', ownedBy: 'mistralai' },
      { id: 'codestral-latest', ownedBy: 'mistralai' },
      { id: 'open-mistral-nemo', ownedBy: 'mistralai' },
    ];
  }
  ...
  private mapError(error: unknown): Error {
    if (error instanceof OpenAI.APIError) {
      if (error.status === 429) {
        return new RateLimitError('mistral', undefined, error);
      }
      if (error.status === 400 && error.message.includes('token')) {
        return new TokenLimitError('mistral', error);
      }
      if (error.status === 502 || error.status === 503) {
        return new ProviderUnavailableError('mistral', error.status, error);
      }
      return new InvalidResponseError('mistral', error.message, error);
    }
