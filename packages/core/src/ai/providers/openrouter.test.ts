...
describe('OpenRouterProvider', () => {
  ...
  describe('constructor', () => {
    it('should throw when no API key is provided', () => {
      ...
      expect(() => new OpenRouterProvider({ model: config.model })).toThrow(
        'OpenRouter provider requires OPENROUTER_API_KEY'
      );
    });

    it('should construct successfully with an API key', () => {
      expect(() => new OpenRouterProvider(makeConfig())).not.toThrow();
    });
  });
  ...
  describe('chat', () => {
    it('should map request and response', async () => {
      ...
      expect(response.id).toBe('chatcmpl-or-123');
  ...
  describe('fetchAvailableModels', () => {
    ...
    it('should return models from OpenRouter endpoint', async () => {
      ...
      const models = await OpenRouterProvider.fetchAvailableModels('test-key');
    ...
    it('should return empty array on fetch failure', async () => {
      ...
      const models = await OpenRouterProvider.fetchAvailableModels('bad-key');
    ...
    it('should return empty array when no key provided', async () => {
      const models = await OpenRouterProvider.fetchAvailableModels();
      expect(models).toEqual([]);
    });
  });

  describe('getKnownModels', () => {
    it('should return known OpenRouter models', () => {
      const models = OpenRouterProvider.getKnownModels();
