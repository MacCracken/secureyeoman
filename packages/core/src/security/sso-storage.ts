...
interface IdpRow {
  ...
  issuer_url: string | null; client_id: string | null; client_secret: string | null;
  scopes: string; metadata_url: string | null; entity_id: string | null; acs_url: string | null;
  enabled: boolean; auto_provision: boolean; default_role: string;
  config: Record<string, unknown>; created_at: number; updated_at: number;
}

function idpFromRow(r: IdpRow): IdentityProvider {
  return {
    id: r.id, name: r.name, type: r.type as 'oidc' | 'saml',
    issuerUrl: r.issuer_url, clientId: r.client_id, clientSecret: r.client_secret,
    scopes: r.scopes, metadataUrl: r.metadata_url, entityId: r.entity_id, acsUrl: r.acs_url,
    enabled: r.enabled, autoProvision: r.auto_provision, defaultRole: r.default_role,
    config: r.config ?? {},
...
interface MappingRow {
  id: string; idp_id: string; local_user_id: string; external_subject: string;
  attributes: Record<string, unknown>; created_at: number; last_login_at: number | null;
}

function mappingFromRow(r: MappingRow): IdentityMapping {
  return {
    id: r.id, idpId: r.idp_id, localUserId: r.local_user_id,
    externalSubject: r.external_subject, attributes: r.attributes ?? {},
...
export class SsoStorage extends PgBaseStorage {
  ...
  async createIdentityProvider(data: IdentityProviderCreate): Promise<IdentityProvider> {
    ...
    await this.execute(
      ...
      [
        id, data.name, data.type, data.issuerUrl, data.clientId, data.clientSecret,
        data.scopes ?? 'openid email profile', data.metadataUrl, data.entityId, data.acsUrl,
        data.enabled ?? true, data.autoProvision ?? true, data.defaultRole ?? 'viewer',
        JSON.stringify(data.config ?? {}), now, now,
      ]
    );
    return { id, ...data, scopes: data.scopes ?? 'openid email profile', config: data.config ?? {}, createdAt: now, updatedAt: now };
  }

  async getIdentityProvider(id: string): Promise<IdentityProvider | null> {
    const row = await this.queryOne<IdpRow>('SELECT * FROM auth.identity_providers WHERE id = $1', [id]);
  ...
  async updateIdentityProvider(id: string, data: IdentityProviderUpdate): Promise<IdentityProvider | null> {
    const updates: string[] = [];
    const values: unknown[] = [];
    let idx = 1;
    const fields: [keyof IdentityProviderUpdate, string][] = [
      ['name', 'name'], ['type', 'type'], ['issuerUrl', 'issuer_url'], ['clientId', 'client_id'],
      ['clientSecret', 'client_secret'], ['scopes', 'scopes'], ['metadataUrl', 'metadata_url'],
      ['entityId', 'entity_id'], ['acsUrl', 'acs_url'], ['enabled', 'enabled'],
      ['autoProvision', 'auto_provision'], ['defaultRole', 'default_role'],
    ];
    for (const [key, col] of fields) {
      if (data[key] !== undefined) { updates.push(`${col} = $${idx++}`); values.push(data[key]); }
    }
    if (data.config !== undefined) { updates.push(`config = $${idx++}::jsonb`); values.push(JSON.stringify(data.config)); }
    if (updates.length === 0) return this.getIdentityProvider(id);
    updates.push(`updated_at = $${idx++}`); values.push(Date.now());
    values.push(id);
    await this.execute(`UPDATE auth.identity_providers SET ${updates.join(', ')} WHERE id = $${idx}`, values);
  ...
  async createIdentityMapping(data: {
    idpId: string; localUserId: string; externalSubject: string;
    attributes?: Record<string, unknown>;
    ...
    await this.execute(
      ...
      [id, data.idpId, data.localUserId, data.externalSubject, JSON.stringify(data.attributes ?? {}), now]
    );
    return { id, idpId: data.idpId, localUserId: data.localUserId, externalSubject: data.externalSubject, attributes: data.attributes ?? {}, createdAt: now, lastLoginAt: null };
  }

  async getMappingByExternalSubject(idpId: string, externalSubject: string): Promise<IdentityMapping | null> {
  ...
  async updateMappingLastLogin(id: string): Promise<void> {
    await this.execute('UPDATE auth.identity_mappings SET last_login_at = $1 WHERE id = $2', [Date.now(), id]);
  }
  ...
  async createSsoState(data: Omit<SsoState, 'createdAt'>): Promise<void> {
    ...
    await this.execute(
      ...
      [data.state, data.providerId, data.redirectUri, data.codeVerifier, data.workspaceId, now, data.expiresAt]
    );
  }

  async getSsoState(state: string): Promise<SsoState | null> {
    const row = await this.queryOne<{
      state: string; provider_id: string; redirect_uri: string; code_verifier: string | null;
      workspace_id: string | null; created_at: number; expires_at: number;
    ...
    return {
      ...
      codeVerifier: row.code_verifier, workspaceId: row.workspace_id,
  ...
  override close(): void { /* pool lifecycle managed globally */ }
}
