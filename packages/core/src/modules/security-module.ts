...
export class SecurityModule implements AppModule {
  ...
  /** True when the encryption key was auto-generated (not user-provided). */
  private encryptionKeyAutoGenerated = false;
  /** True when the JWT token secret was auto-generated (not user-provided). */
  private tokenSecretAutoGenerated = false;
  /** True when the audit signing key was auto-generated (not user-provided). */
  private signingKeyAutoGenerated = false;
  /** True when the CI/CD webhook secret was auto-generated. */
  private webhookSecretAutoGenerated = false;
  ...
  // --- MCP credential re-encryption (set lazily after gateway init) ---
  private mcpCredentialManager: McpCredentialManager | null = null;
  ...
  async initEarly(): Promise<void> {
    ...
    // Auto-generate cryptographic keys when not provided externally.
    // Each uses randomBytes(32) for 256-bit entropy, stored via SecretsManager
    // (or process.env as fallback). User-provided values are always preserved.
    const { randomBytes } = await import('node:crypto');

    // Encryption key
    const encKeyEnv = this.config.security.encryption.keyEnv;
    if (this.config.security.encryption.enabled && !getSecret(encKeyEnv)) {
      const generated = randomBytes(32).toString('base64url');
      if (this.secretsManager) {
        await this.secretsManager.set(encKeyEnv, generated);
      } else {
        process.env[encKeyEnv] = generated;
      }
      this.encryptionKeyAutoGenerated = true;
      this.logger.info('Auto-generated encryption key (no external key provided)');
    }

    // JWT token secret
    const tokenSecretEnv = this.config.gateway.auth.tokenSecret;
    if (!getSecret(tokenSecretEnv)) {
      const generated = randomBytes(32).toString('base64url');
      if (this.secretsManager) {
        await this.secretsManager.set(tokenSecretEnv, generated);
      } else {
        process.env[tokenSecretEnv] = generated;
      }
      this.tokenSecretAutoGenerated = true;
    ...
    if (!getSecret(signingKeyEnv)) {
      const generated = randomBytes(32).toString('base64url');
      if (this.secretsManager) {
        await this.secretsManager.set(signingKeyEnv, generated);
      } else {
        process.env[signingKeyEnv] = generated;
      }
      this.signingKeyAutoGenerated = true;
      this.logger.info('Auto-generated audit signing key (no external key provided)');
    }

    // CI/CD inbound webhook HMAC verification secret
    if (!getSecret('SECUREYEOMAN_WEBHOOK_SECRET')) {
      const generated = randomBytes(32).toString('base64url');
      if (this.secretsManager) {
        await this.secretsManager.set('SECUREYEOMAN_WEBHOOK_SECRET', generated);
      } else {
        process.env.SECUREYEOMAN_WEBHOOK_SECRET = generated;
      }
      this.webhookSecretAutoGenerated = true;
      this.logger.info('Auto-generated CI/CD webhook secret (no external key provided)');
    }
  ...
  async initPostAuth(deps: SecurityPostAuthDeps): Promise<void> {
    ...
    if (this.config.security.rotation.enabled) {
      ...
      const secretDefs: SecretMetadata[] = [
        {
          ...
          createdAt: now,
          expiresAt: this.tokenSecretAutoGenerated ? now + 30 * 86_400_000 : null,
          rotatedAt: null,
          rotationIntervalDays: this.tokenSecretAutoGenerated ? 30 : null,
          autoRotate: this.tokenSecretAutoGenerated,
          source: this.tokenSecretAutoGenerated ? 'internal' : 'external',
          category: 'jwt',
        },
        {
          name: this.config.logging.audit.signingKeyEnv,
          createdAt: now,
          expiresAt: this.signingKeyAutoGenerated ? now + 90 * 86_400_000 : null,
          rotatedAt: null,
          rotationIntervalDays: this.signingKeyAutoGenerated ? 90 : null,
          autoRotate: this.signingKeyAutoGenerated,
          source: this.signingKeyAutoGenerated ? 'internal' : 'external',
        ...
        },
        {
          ...
          createdAt: now,
          expiresAt: this.encryptionKeyAutoGenerated ? now + 90 * 86_400_000 : null,
          rotatedAt: null,
          rotationIntervalDays: this.encryptionKeyAutoGenerated ? 90 : null,
          autoRotate: this.encryptionKeyAutoGenerated,
          source: this.encryptionKeyAutoGenerated ? 'internal' : 'external',
          category: 'encryption',
        },
        {
          name: 'SECUREYEOMAN_WEBHOOK_SECRET',
          createdAt: now,
          expiresAt: this.webhookSecretAutoGenerated ? now + 90 * 86_400_000 : null,
          rotatedAt: null,
          rotationIntervalDays: this.webhookSecretAutoGenerated ? 90 : null,
          autoRotate: this.webhookSecretAutoGenerated,
          source: this.webhookSecretAutoGenerated ? 'internal' : 'external',
          category: 'api_key',
        },
      ];
      ...
      const rotMgr = this.rotationManager;
      ...
      this.rotationManager.setCallbacks({
        onRotate: async (name, newValue) => {
          ...
          if (name === tokenSecretEnv) {
            ...
            // Re-encrypt MCP credentials with the new derived key
            if (this.mcpCredentialManager) {
              try {
                const oldSecret = await rotMgr.getPreviousValue(name);
                if (oldSecret) {
                  const count = await this.mcpCredentialManager.reEncrypt(oldSecret, newValue);
                  this.logger?.info(
                    { reEncrypted: count },
                    'MCP credentials re-encrypted after TOKEN_SECRET rotation'
                  );
                } else {
                  // No previous value available — just update the internal key
                  this.mcpCredentialManager.updateEncryptionKey(newValue);
                  this.logger?.warn(
                    'No previous TOKEN_SECRET available for MCP credential re-encryption; updated key only'
                  );
                }
              } catch (err) {
                this.logger?.warn(
                  { error: String(err) },
                  'MCP credential re-encryption failed — credentials may need manual re-entry'
                );
                // Update internal key anyway so new credentials use the correct key
                this.mcpCredentialManager.updateEncryptionKey(newValue);
              }
            }
  ...
  /**
   * Register the MCP credential manager so that TOKEN_SECRET rotation
   * can re-encrypt stored credentials. Called lazily after gateway init.
   */
