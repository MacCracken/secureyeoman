...
SecureYeoman exposes a comprehensive Model Context Protocol (MCP) server with 330 tools, 9 resources, and 4 prompts. This ADR consolidates decisions governing the MCP service architecture, tool registration, transport layer, tool categories, and context optimization.
...
**Auth Delegation.** The MCP service requires a live SecureYeoman core instance. On startup, the service self-mints a service JWT using the shared `SECUREYEOMAN_TOKEN_SECRET` (HS256, `sub: "mcp-service"`, `role: "admin"`, 365-day expiry). User-facing requests are authenticated by delegating JWT validation to `POST /api/v1/auth/verify` on the core gateway. RBAC permissions enforced per-tool. Every tool call logged to the audit chain. All tool outputs pass through `secret-redactor` middleware.
